...
\begin{tikzpicture}[node distance=0.6cm, >=stealth,
  ...
  \node[resultbox, below=of agg, xshift=-1.4cm] (val) {Human Validation\\$n{=}50$, AUC{=}0.795};
...
\begin{tabular}{lrrr}
\toprule
\textbf{Dimension} & \textbf{AUC-ROC} & \textbf{F1} & \textbf{Cohen's $\kappa$} \\
\midrule
...
Validation results (Table~\ref{tab:validation}) demonstrated that the combined multimodal pipeline achieved strong discriminative ability, with an overall AUC-ROC of $0.795$ and a significant rank correlation with human judgments (Spearman $\rho = 0.455$, $p=0.002$). For the most prevalent dimension, performative labor, the pipeline performed exceptionally well (AUC-ROC $= 0.915$, $\kappa = 0.594$). While threshold-dependent metrics like Cohen's $\kappa$ varied across dimensions due to class imbalance and differing subjective thresholds, the high AUC-ROC scores indicate that the pipeline effectively ranks videos by their degree of exploitation. Future work should employ multiple independent annotators to establish inter-rater reliability.
