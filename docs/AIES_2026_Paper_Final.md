...
**VLM Multimodal Classification (Weight = 0.67):** We deployed the GPT-4.1-mini Vision API to analyze the combination of the video's title, thumbnail image, and description. The VLM provided a continuous probability score ($[0, 1]$) for each dimension. Validation on a subset of 50 human-annotated videos demonstrated that the combined multimodal pipeline achieved strong discriminative ability, with an AUC-ROC of $0.795$ and a significant rank correlation with human judgments (Spearman $\rho = 0.455$, $p=0.0019$). When discretized at the optimal threshold, the pipeline achieved an F1-score of $0.760$ and moderate-to-substantial agreement (Cohen's $\kappa = 0.462$). For the most prevalent dimension, performative labor, the pipeline performed exceptionally well (AUC-ROC $= 0.915$, $\kappa = 0.650$), justifying the higher weight for visual and descriptive signals in the final aggregation.
