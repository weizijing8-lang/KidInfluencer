...
\PARstart{T}{he} commercialization of childhood through digital platforms has created a lucrative ``kidfluencer'' economy in which children are featured in YouTube videos---unboxing toys, participating in challenges, performing scripted roleplay, and documenting their daily lives \cite{clark2025child}. This phenomenon has sparked intense debate regarding the ethics of child digital labor, the psychological impact of public exposure, and the blurring of lines between play and work, often termed ``playbour'' \cite{abidin2015micromicrocelebrity}.
...
Crucially, because we rely on observational data via the YouTube API, we cannot directly observe the recommendation algorithm's internal scoring \cite{bouchaud2024auditing,rieder2018ranking}. Instead, we measure the \textbf{engagement premium}---the association between exploitative content dimensions and view counts---which serves as a proxy for the incentive structures shaping creator behavior. We emphasize that all findings reported in this study are \textbf{correlational}: while we control for channel-level confounds and video age, we cannot establish causal claims about whether exploitation \emph{causes} higher engagement, or whether unobserved confounders (e.g., higher production quality co-occurring with exploitation signals) partially explain the association.
...
Traditional machine learning requires massive labeled datasets, which are difficult to obtain for subjective concepts. Weak supervision frameworks like Snorkel \cite{ratner2017snorkel,ratner2016data,bach2019snorkel} allow researchers to encode domain knowledge as noisy heuristic rules (Labeling Functions) to generate probabilistic labels \cite{johnson2022survey}. Recently, Large Language Models (LLMs) and Vision-Language Models (VLMs) have shown promise in zero-shot content moderation and annotation \cite{ma2023adapting,gilardi2023chatgpt,tornberg2024chatgpt}. Comparative evaluations across classical and deep learning architectures further demonstrate that model selection and feature representation significantly impact classification performance on user-generated text \cite{tian2025empirical}, while controlled studies on toxic comment classification show that integrating social metadata features can bridge the gap between lightweight models and computationally expensive Transformers \cite{ding2026feature}. We combine LLMs and VLMs within a weak supervision framework to scale our exploitation analysis across text and visual modalities.
...
The multimodal pipeline successfully processed 4,208 videos from 56 kid-centric channels. Overall, 26.5\% of the sampled videos ($n=1,114$) were classified as exploitative (score $\ge 0.7$). Fig.~\ref{fig:score_dist} shows the distribution of exploitation risk scores across the dataset.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig1_score_distribution.png}
\caption{Distribution of overall exploitation risk scores across 4,208 videos from 56 kid-centric channels. The dashed red line indicates the classification threshold ($\ge 0.7$); 26.5\% of videos exceed this threshold.}
...
Fig.~\ref{fig:dimension_premiums} visualizes the engagement premium structure. The results reveal a clear engagement premium across channels with sufficient data:

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig2_dimension_premiums.png}
\caption{Within-channel engagement premium by exploitation dimension. Bars show the percentage difference in median views between videos exhibiting each dimension and videos from the same channel lacking it. Significance: $^{***}p<0.001$ (FDR-corrected); n.s.~= not significant.}
\label{fig:dimension_premiums}
\end{figure}
\begin{itemize}
...
Crucially, \textbf{Commercial Content} exhibited a slight \textbf{negative} premium ($-3.8\%$, FDR $p=0.516$). While not statistically significant after FDR correction, the directionality suggests that videos featuring explicit product placements do not benefit from the massive engagement boosts seen in emotional and performative exploitation dimensions. Fig.~\ref{fig:emotional_vs_commercial} contrasts the channel-level patterns for emotional bait (consistent premium) versus commercial content (no premium).

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig5_emotional_vs_commercial.png}
\caption{Channel-level pairwise comparison of mean $\log_{10}(\text{views})$ for emotional bait (left, blue) and commercial content (right, red). Each point represents one channel. Points above the diagonal indicate a positive engagement premium. Emotional bait shows a consistent premium across nearly all channels, while commercial content clusters around the diagonal (no premium).}
\label{fig:emotional_vs_commercial}
\end{figure}
...
Additionally, we verified the robustness of our results by analyzing the overall exploitation flag at the channel level. Across 47 channels with sufficient variance, 36 channels (76.6\%) exhibited a positive engagement premium for high-exploitation videos, yielding an average within-channel boost of $+32.8\%$ (Wilcoxon $p=0.001$). Fig.~\ref{fig:channel_premiums} shows the per-channel distribution of this premium. This confirms that the performativity premium is a widespread structural phenomenon, not driven by a few outlier channels.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig4_channel_premiums.png}
\caption{Within-channel exploitation premium for each of 47 channels with sufficient variance. Blue bars indicate channels where exploitative videos receive more views; red bars indicate the opposite. 76.6\% of channels exhibit a positive premium.}
\label{fig:channel_premiums}
\end{figure}

We note that these results are correlational. A plausible confounder is \textbf{production quality}: channels that invest heavily in scripted content (triggering our performative labor and narrative conflict dimensions) may also invest in higher-quality editing, lighting, and thumbnails, which independently attract views. Our within-channel design partially addresses this concern---comparing videos from the same channel holds production resources approximately constant---but we cannot fully rule out within-channel variation in production effort. Future work incorporating direct measures of production quality (e.g., video resolution, cut frequency, thumbnail A/B testing) could help disentangle these effects.
...
The pipeline source code, labeling functions, and video-level metadata (video IDs, exploitation risk scores, and engagement metrics) are publicly available at \url{https://github.com/weizijing8-lang/KidInfluencer}. Raw thumbnail images are not redistributed to protect minors' privacy.
